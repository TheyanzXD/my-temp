...
export const GET: RequestHandler = async ({ getClientAddress, platform }) => {
	...
	try {
		const domains = await getDomains(platform);
		return json(
			{ success: true, data: { domains } },
			{ headers: { ...securityHeaders, 'Cache-Control': 'public, max-age=60, s-maxage=300' } }
		);
	} catch (err: unknown) {
