...
export const GET: RequestHandler = async ({ params, getClientAddress, platform }) => {
	...
	if (!address || !id) {
		return json(
			{ success: false, error: { code: 'INVALID_REQUEST', message: 'Missing address or message ID' } },
			{ status: 400, headers: securityHeaders }
		);
	}

	try {
		const message = await getMessage(platform, address, id);
		if (!message) {
			return json(
...
export const DELETE: RequestHandler = async ({ params, getClientAddress, platform }) => {
	...
	try {
		const deleted = await deleteMessage(platform, address, id);
		return json({ success: true, data: { deleted, id } }, { headers: securityHeaders });
	} catch (err: unknown) {
